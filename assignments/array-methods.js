...
runners.forEach(function(index) {
   fullNames.push(index.first_name + " " + index.last_name);
});
...
runners.map(index => firstNamesAllCaps.push(index.first_name.toUpperCase()) );
/*runners.forEach(function(index) {
   firstNamesAllCaps.push(index.first_name.toUpperCase());
});*/
...
runnersLargeSizeShirt = runners.filter(index => index.shirt_size === "L");
/*runners.forEach(function(index) {
  if (index.shirt_size === "L") {
    runnersLargeSizeShirt.push(index);
  }
});*/
...
/*
const reducer = (ticketPriceTotal, index) => ticketPriceTotal = ticketPriceTotal + index.donation; // would treat as strings, and skip index 0?
...
const reducer = (accumulator, currentValue) => ({donation: accumulator.donation + currentValue.donation});
ticketPriceTotal = runners.reduce(reducer).donation;
/*IF I DO THIS OTHER THAN OBJ VALUE + OBJ VALUE I get NAN?*/
/*runners.forEach(function(index) {
   ticketPriceTotal = ticketPriceTotal + index.donation;
});
*/
...
let runnersEmail = [];
runners.forEach(function(index) {
   runnersEmail.push(index.first_name + ":" + index.email);
});
console.log(runnersEmail);


// Problem 2
// companie names in alfa order
let companynames = [];
runners.forEach(function(index) {
   companynames.push(index.company_name);
});
console.log(companynames.sort());

// Problem 3
// ID and last name for takgs
let idAndnames = [];
runners.forEach(function(index) {
   idAndnames.push(index.id + ":" + index.last_name);
});
